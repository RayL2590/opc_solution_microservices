...
/**
 * Pure-unit oracle for the front-service security beans (Story 5.1, FR-16 / NFR-S2).
 *
 * <p>No Spring context, no {@code @WebMvcTest}, no DataSource — instantiates
 * {@link SecurityConfig} directly and calls its {@code @Bean} methods. The 401/redirect
 * filter-chain wiring is exercised by the {@code @WebMvcTest} slice ({@code HomeControllerTest});
 * this test only inspects the seeded user store. Mirrors patient-service's {@code SecurityConfigTest}
 * (Story 2.4).
 *
 * <p>Oracles:
 * <ul>
 *   <li>The seeded password is stored as a BCrypt hash ({@code $2…}), never plaintext.</li>
 *   <li>The hash is stored <em>verbatim</em> — the raw demo password matches it (proving it was
 *       not re-encoded; {@code encoder.encode(hash)} would double-hash and break login), and the
 *       old scaffold password ({@code user123}) does not match.</li>
 *   <li>The {@link PasswordEncoder} bean is a {@link BCryptPasswordEncoder} (not a NoOp).</li>
 * </ul>
 */
class SecurityConfigTest {

    /** Demo credential (DEV/eval only) — the same identity the Gateway and patient-service commit. */
